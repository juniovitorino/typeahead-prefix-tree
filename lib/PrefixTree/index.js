...
class Tree {
  ...
  filterByPopularity(recordsLimit) {
    this.resetPreviewsSearchs()
    this.filterTreeRecordsByRoot(this.root)
    return this.prepareRecords(recordsLimit)
  }

  resetPreviewsSearchs() {
    this.records = []
    this.searchPrefix = ''
    this.exactMatchRecord = undefined
  }
  ...
  updateRecords({ key, times, end }) {
    if (key && this.normalizeKey(this.searchPrefix) === this.normalizeKey(key)) {
  ...
  setup(key) {
    this.exactMatchRecord = undefined
    try {
      return { root: this.root, cleanKey: this.normalizeKey(key) }
    } catch (e) {
  ...
  normalizeKey(string) {
    return string.toLowerCase().replaceAll(/\s/g, '_').trim()
  }
